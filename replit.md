...
- **Collaboration**: Multi-user system with stock-specific comment threads, interest markers, recommendation filtering, and personal stock pinning for quick access.
- **Stock Pinning**: Users can pin individual stocks to a dedicated "Pinned" tab for quick access. Pin state is integrated into the main stocks query (LEFT JOIN on user_stock_pins) to avoid N+1 queries. Pin buttons feature optimistic updates for instant feedback with proper cache invalidation for multi-device consistency.
...
- **Follow Stock System**: Comprehensive follow/unfollow functionality with duplicate prevention. Backend validates against existing follows and returns 409 status for duplicates. Frontend displays appropriate Follow/Unfollow buttons on ticker detail pages based on current follow status. Bulk follow operations filter out already-followed stocks and provide detailed feedback about partial selections. Day-0 AI analysis automatically triggered when following new stocks.
